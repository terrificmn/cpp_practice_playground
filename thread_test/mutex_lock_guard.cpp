...
std::list<int> myList;
std::mutex myMutex; //global로 선언

// list에 추가를 하는데 파라미터로 들어온 값을 나머지가 0이면 list에 추가
void addToList(int max, int interval) {
    // addToList 함수에 연결할 수 있게 lock_guard를 설정
...
void printList() {
    // printList 함수에 연결할 수 있게 lock_guard를 설정
    std::lock_guard<std::mutex> guard(myMutex);
...
int main() {
    ...
    // 어쨋든 multithread 예제  (잘 나오는게 함정;;;)

    // 사실 여기저기에서 multi thread로 사용하다보면 data가 보호가 안되고 문제가 있어서 
    // 이것을 해결하는 것이 std::mutex 임

    // mutex를 include하고 멤버 함수 인 lock()을 호출하고, unlock()을 호출할 수 있으나  
    // 이때에는 함수 밖에서 unlock()을 해야한다고 한다  
    // 그래서 표준 c++ 라이브러리에서는 std::lock_guard class template를 제공, 
    // 생성될 때 lock을 mutex에 걸어주고, 소멸할 때 unlock 시켜준다고 함  

    // 그래서 실행 결과가 뒤섞이지 않고 잘 나오게 된다. 
    return 0;
}